...
class App:
        ...
        for row in self.cells:
            ...
            for cell in row:
                if cell.get() not in valid_entries:
                    self.invalid_entry_popup()
                    break
                elif cell.get() == "":
                    row_values.append(0)
                else:
                    row_values += int(cell.get())
    ...
    def invalid_entry_popup(self):
        '''Produces a pop-up for indicating there is an invalid board entry
        '''
        popup = Tk()
        ...
        label = ttk.Label(popup, text="Invalid entries!")
        label.pack()
